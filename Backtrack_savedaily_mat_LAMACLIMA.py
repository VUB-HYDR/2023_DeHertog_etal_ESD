...
# create datelist
if model !='cesm':
    datelist = get_times_daily(dt.date(years[0],months[0],1), dt.date(years[-1],months[-1], end_day))[::-1]
    datelist=remove_leap_days(datelist)
else:
# create datelist without the if statement of Missisipi as there are no leap year
    datelist = get_times_daily(dt.date(years[0],months[0],start_day), dt.date(years[-1],months[-1], end_day))[::-1]
...
def get_Sa_track_backward(latitude,longitude,count_time,divt,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
    ...
    # make P_region matrix. Extract P only for the land 
     ...
     #   west_loss[t-1,0,:] = (Fa_W_top_WE[t-1,:,1] * (Sa_track_top[t,:,1] / W_top[t,:,1])
      #                          + Fa_W_down_WE[t-1,:,1] * (Sa_track_down[t,:,1] / W_down[t,:,1]))
        ...
        down_to_top[t-1,:,:] = np.reshape(np.maximum(0, np.reshape(Sa_track_after_Fa_P_E_down, (np.size(Sa_track_after_Fa_P_E_down))) - np.reshape(W_down[t-1,:,:],
                                            (np.size(W_down[t-1,:,:])))), (len(latitude),len(longitude))) # Imme: invalid value encountered in maximum
        top_to_down[t-1,:,:] = np.reshape(np.maximum(0, np.reshape(Sa_track_after_Fa_P_E_top, (np.size(Sa_track_after_Fa_P_E_top))) - np.reshape(W_top[t-1,:,:],
                                            (np.size(W_top[t-1,:,:])))), (len(latitude),len(longitude))) # Imme: invalid value encountered in maximum
...
for date in datelist[1:]:
    ...
    if (calendar.isleap(date.year)==1) and (str(date.month)=='2') and (str(date.day)=='28'):
...
#                                        timestep,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,Fa_Vert,E,P,W_top,W_down,Sa_track_top_last,Sa_track_down_last,Sa_time_top_last,Sa_time_down_last)
    ...
    np.savez_compressed(datapath[3], Sa_track_top_last = Sa_track_top[0,:,:], Sa_track_down_last = Sa_track_down[0,:,:], E_per_day = E_per_day, E_track_per_day = E_track_per_day, P_per_day = P_per_day, \

...
sub_interdata_folder = os.path.join(interdata_folder, 'Regional_scale_forward_daily') # Insert sub-interdata folder here
...
def data_path(y,a,month,years,timetracking):
    load_Sa_track = os.path.join(sub_interdata_folder, str(y).zfill(4) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) +  'Sa_reg.npz')
    ...
    save_path = os.path.join(output_folder, 'P_track_regional_scales_full' + str(years[0]) + '-' + str(years[-1]) + '-timetracking' + str(timetracking))
    
    save_path_daily = os.path.join(output_folder, 'P_track_regional_scales_daily_full' + str(y) + '-timetracking' + str(timetracking))
...
for year in years[:]:
    ...
    for i,date in enumerate(datelist):
        ...
        if i > final_time: # a = 365 (366th index) and not a leapyear\
            ...
            P_track_per_day[i,:,:] = loading_ST['P_reg_per_day']
            P_per_day[i,:,:] = loading_ST['P_per_day']
            Sa_track_down_per_day[i,:,:] = loading_ST['Sa_reg_down_per_day']
            Sa_track_top_per_day[i,:,:] = loading_ST['Sa_reg_top_per_day']

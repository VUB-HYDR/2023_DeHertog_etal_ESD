...
# This Backreg script is based on the Con_E_Recyc_Masterscript.py from the WAM-2layers model from Ruud van der Ent but adapted to fit the purposes of the study by Imme Benedict
...
# Timereging was not implemented in this code
...
Region = lsm # region to perform the reging for
...
timereging = 0 # 0 for not reging time and 1 for reging time
...
sub_interdata_folder = os.path.join(interdata_folder, 'Regional_scale_forward_daily')
...
def data_path_ea(year, month, day):
    save_empty_arrays_reg = os.path.join(sub_interdata_folder, str(year).zfill(4) + '-' + str(month).zfill(2) + '-' + str(day).zfill(2) + 'Sa_reg')
    save_empty_arrays_time = os.path.join(sub_interdata_folder, str(year).zfill(4) + '-' +str(month).zfill(2) + '-' + str(day).zfill(2) + 'Sa_time')
    return save_empty_arrays_reg,save_empty_arrays_time

#define the function for loading previous data 
def data_path(previous_data_to_load,yearnumber,month,a):
    load_Sa_reg = os.path.join(sub_interdata_folder, str(previous_data_to_load.year).zfill(4) + '-' + str(previous_data_to_load.month).zfill(2) + '-' + str(previous_data_to_load.day).zfill(2) + 'Sa_reg.npz')  
    ...
    save_path_reg = os.path.join(sub_interdata_folder, str(yearnumber).zfill(4) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'Sa_reg')
    save_path_time = os.path.join(sub_interdata_folder, str(yearnumber).zfill(4) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'Sa_time')
    return load_Sa_reg,load_fluxes_and_storages,load_Sa_time,save_path_reg,save_path_time

#def data_path(yearnumber,month,a):
    #load_Sa_reg = os.path.join(sub_interdata_folder, str(previous_data_to_load.year) + '-' + str(previous_data_to_load.month).zfill(2) + '-' + str(previous_data_to_load.day).zfill(2) + 'Sa_reg.npz')    
 #   load_fluxes_and_storages = os.path.join(interdata_folder, str(yearnumber).zfill(4) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'fluxes_storages.mat')
...
#    load_Sa_time = os.path.join(sub_interdata_folder, str(previous_data_to_load.year) + '-' + str(previous_data_to_load.month).zfill(2) + '-' + str(previous_data_to_load.day).zfill(2) + 'Sa_time.npz')
  ...
  #  save_path_reg = os.path.join(sub_interdata_folder, str(yearnumber).zfill(4) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'Sa_reg')
  #  save_path_time = os.path.join(sub_interdata_folder, str(yearnumber).zfill(4) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'Sa_time')
   # return load_fluxes_and_storages,save_path_reg,save_path_time
#%% Code

def get_Sa_reg_forward(latitude,longitude,count_time,divt,Kvf,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
                                       Fa_Vert,E,P,W_top,W_down,Sa_reg_top_last,Sa_reg_down_last):
   ...
   # Fa_N_down_boundary = np.zeros(np.shape(Fa_N_down)); # als je niet met np.nan multiplied krijg je in de volgende alinea geen invalid value encountered in less
    ...
    Sa_reg_down = np.zeros(np.shape(W_down))
    Sa_reg_top = np.zeros(np.shape(W_top))
    
    # assign begin values of output == last (but first index) values of the previous time slot
    Sa_reg_down[0,:,:] = Sa_reg_down_last
    Sa_reg_top[0,:,:] = Sa_reg_top_last
    
    # defining sizes of reged moisture (one timestep with structure[1,192,288])
    Sa_reg_after_Fa_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_reg_after_Fa_P_E_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_reg_E_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_reg_W_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_reg_N_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_reg_S_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_reg_after_Fa_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_reg_after_Fa_P_E_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_reg_E_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_reg_W_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_reg_N_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_reg_S_top = np.zeros(np.shape(Sa_reg_top_last))

    # define sizes of total moisture (one timestep)
    Sa_E_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_W_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_N_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_S_down = np.zeros(np.shape(Sa_reg_down_last))
    Sa_E_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_W_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_N_top = np.zeros(np.shape(Sa_reg_top_last))
    Sa_S_top = np.zeros(np.shape(Sa_reg_top_last))
    ...
    for t in range(np.int(count_time*divt)):
        ...
        Sa_S_top[0,:-1,:] = W_top[t,1:,:] # Atmospheric storage of the cell to the south [m3]
         ...
         # down: define values of reged moisture of neighbouring grid cells
        Sa_reg_E_down[0,:,:-1] = Sa_reg_down[t,:,1:] # Atmospheric reged storage of the cell to the east [m3]
        if isglobal == 1:       
            Sa_reg_E_down[0,:,-1] = Sa_reg_down[t,:,0] #Atmospheric reged storage of the cell to the east [m3]
        Sa_reg_W_down[0,:,1:] = Sa_reg_down[t,:,:-1] # Atmospheric storage of the cell to the west [m3]
        if isglobal == 1:       
            Sa_reg_W_down[0,:,0] = Sa_reg_down[t,:,-1] # Atmospheric storage of the cell to the west [m3]
        Sa_reg_N_down[0,1:,:] = Sa_reg_down[t,:-1,:] # Atmospheric storage of the cell to the north [m3]
        Sa_reg_S_down[0,:-1,:] = Sa_reg_down[t,1:,:] # Atmospheric storage of the cell to the south [m3]
        
        # down: calculate with moisture fluxes
        Sa_reg_after_Fa_down[0,1:-1,:] = (Sa_reg_down[t,1:-1,:] 
        - Fa_E_down_WE[t,1:-1,:] * (Sa_reg_down[t,1:-1,:] / W_down[t,1:-1,:])
#        + Fa_E_down_EW[t,1:-1,:] * (Sa_reg_E_down[0,1:-1,:] / Sa_E_down[0,1:-1,:]) 
 #       + Fa_W_down_WE[t,1:-1,:] * (Sa_reg_W_down[0,1:-1,:] / Sa_W_down[0,1:-1,:]) 
        - Fa_W_down_EW[t,1:-1,:] * (Sa_reg_down[t,1:-1,:] / W_down[t,1:-1,:]) 
        - Fa_N_down_SN[t,1:-1,:] * (Sa_reg_down[t,1:-1,:] / W_down[t,1:-1,:]) 
 #       + Fa_N_down_NS[t,1:-1,:] * (Sa_reg_N_down[0,1:-1,:] / Sa_N_down[0,1:-1,:])
 #       + Fa_S_down_SN[t,1:-1,:] * (Sa_reg_S_down[0,1:-1,:] / Sa_S_down[0,1:-1,:]) 
        - Fa_S_down_NS[t,1:-1,:] * (Sa_reg_down[t,1:-1,:] / W_down[t,1:-1,:]) 
        + Fa_downward[t,1:-1,:] * (Sa_reg_top[t,1:-1,:] / W_top[t,1:-1,:])
        - Fa_upward[t,1:-1,:] * (Sa_reg_down[t,1:-1,:] / W_down[t,1:-1,:]))
         # sometimes you get a Runtimewarning: invalid value encountered in less  
        # RuntimeWarning: invalid value encountered in divide
        # ik kan me voorstellen dat die error er komt als er een nan in Sa_reg_down zit en dat je daar dan door moet gaan delen..
        
        # top: define values of reged moisture of neighbouring grid cells
        Sa_reg_E_top[0,:,:-1] = Sa_reg_top[t,:,1:] # Atmospheric reged storage of the cell to the east [m3]
        if isglobal == 1:       
            Sa_reg_E_top[0,:,-1] = Sa_reg_top[t,:,0] # Atmospheric reged storage of the cell to the east [m3]
        Sa_reg_W_top[0,:,1:] = Sa_reg_top[t,:,:-1] # Atmospheric reged storage of the cell to the west [m3]
        if isglobal == 1:       
            Sa_reg_W_top[0,:,0] = Sa_reg_top[t,:,-1] # Atmospheric reged storage of the cell to the west [m3]
        Sa_reg_N_top[0,1:,:] = Sa_reg_top[t,:-1,:] # Atmospheric reged storage of the cell to the north [m3]
        Sa_reg_S_top[0,:-1,:] = Sa_reg_top[t,1:,:] # Atmospheric reged storage of the cell to the south [m3]
        
        # top: calculate with moisture fluxes 
        Sa_reg_after_Fa_top[0,1:-1,:] = (Sa_reg_top[t,1:-1,:] 
        - Fa_E_top_WE[t,1:-1,:] * (Sa_reg_top[t,1:-1,:] / W_top[t,1:-1,:])  
  #      + Fa_E_top_EW[t,1:-1,:] * (Sa_reg_E_top[0,1:-1,:] / Sa_E_top[0,1:-1,:]) 
  #      + Fa_W_top_WE[t,1:-1,:] * (Sa_reg_W_top[0,1:-1,:] / Sa_W_top[0,1:-1,:]) 
        - Fa_W_top_EW[t,1:-1,:] * (Sa_reg_top[t,1:-1,:] / W_top[t,1:-1,:]) 
        - Fa_N_top_SN[t,1:-1,:] * (Sa_reg_top[t,1:-1,:] / W_top[t,1:-1,:]) 
  #      + Fa_N_top_NS[t,1:-1,:] * (Sa_reg_N_top[0,1:-1,:] / Sa_N_top[0,1:-1,:]) 
  #      + Fa_S_top_SN[t,1:-1,:] * (Sa_reg_S_top[0,1:-1,:] / Sa_S_top[0,1:-1,:]) 
        - Fa_S_top_NS[t,1:-1,:] * (Sa_reg_top[t,1:-1,:] / W_top[t,1:-1,:]) 
        - Fa_downward[t,1:-1,:] * (Sa_reg_top[t,1:-1,:] / W_top[t,1:-1,:]) 
        + Fa_upward[t,1:-1,:] * (Sa_reg_down[t,1:-1,:] / W_down[t,1:-1,:]))
         
        # losses to the north and south
        north_loss[t,0,:] = (Fa_N_top_SN[t,1,:] * (Sa_reg_top[t,1,:] / W_top[t,1,:])
                            + Fa_N_down_SN[t,1,:] * (Sa_reg_down[t,1,:] / W_down[t,1,:]))
        south_loss[t,0,:] = (Fa_S_top_NS[t,-2,:] * (Sa_reg_top[t,-2,:] / W_top[t,-2,:])
                            + Fa_S_down_NS[t,-2,:] * (Sa_reg_down[t,-2,:] / W_down[t,-2,:]))

        # Imme added: losses to the east and west
   #     east_loss[t-1,0,:] = (Fa_E_top_EW[t-1,:,-2] * (Sa_reg_top[t,:,-2] / W_top[t,:,-2])
    #                            + Fa_E_down_EW[t-1,:,-2] * (Sa_reg_down[t,:,-2] / W_down[t,:,-2]))
     #   west_loss[t-1,0,:] = (Fa_W_top_WE[t-1,:,1] * (Sa_reg_top[t,:,1] / W_top[t,:,1])
      #                          + Fa_W_down_WE[t-1,:,1] * (Sa_reg_down[t,:,1] / W_down[t,:,1]))

        # down: substract precipitation and add evaporation
        Sa_reg_after_Fa_P_E_down[0,1:-1,:] = (Sa_reg_after_Fa_down[0,1:-1,:]
                                                     - P[t,1:-1,:] * (Sa_reg_down[t,1:-1,:] / W[t,1:-1,:]) 
                                                    + E[t,1:-1,:])

        # top: substract precipitation
        Sa_reg_after_Fa_P_E_top[0,1:-1,:] = (Sa_reg_after_Fa_top[0,1:-1,:] 
                                                - P[t,1:-1,:] * (Sa_reg_top[t,1:-1,:] / W[t,1:-1,:])) 
        
        # down and top: redistribute unaccounted water that is otherwise lost from the sytem
        down_to_top[t,:,:] = np.reshape(np.maximum(0, np.reshape(Sa_reg_after_Fa_P_E_down, (np.size(Sa_reg_after_Fa_P_E_down))) - np.reshape(W_down[t+1,:,:],
                                            (np.size(W_down[t+1,:,:])))), (len(latitude),len(longitude)))
        top_to_down[t,:,:] = np.reshape(np.maximum(0, np.reshape(Sa_reg_after_Fa_P_E_top, (np.size(Sa_reg_after_Fa_P_E_top))) - np.reshape(W_top[t+1,:,:],
                                            (np.size(W_top[t+1,:,:])))), (len(latitude),len(longitude)))
        Sa_reg_after_all_down = Sa_reg_after_Fa_P_E_down - down_to_top[t,:,:] + top_to_down[t,:,:]
        Sa_reg_after_all_top = Sa_reg_after_Fa_P_E_top - top_to_down[t,:,:] + down_to_top[t,:,:]

        # down and top: water lost to the system: 
        water_lost_down[t,:,:] = np.reshape(np.maximum(0, np.reshape(Sa_reg_after_all_down, (np.size(Sa_reg_after_all_down))) - np.reshape(W_down[t+1,:,:],
                                            (np.size(W_down[t+1,:,:])))), (len(latitude),len(longitude)))
        water_lost_top[t,:,:] = np.reshape(np.maximum(0, np.reshape(Sa_reg_after_all_top, (np.size(Sa_reg_after_all_top))) - np.reshape(W_top[t+1,:,:],
                                            (np.size(W_top[t+1,:,:])))), (len(latitude),len(longitude)))
        water_lost[t,:,:] = water_lost_down[t,:,:] + water_lost_top[t,:,:]

        # down: determine Sa_region of this next timestep 100% stable
        Sa_reg_down[t+1,1:-1,:] = np.reshape(np.maximum(0,np.minimum(np.reshape(W_down[t+1,1:-1,:], np.size(W_down[t+1,1:-1,:])), np.reshape(Sa_reg_after_all_down[0,1:-1,:],
                                                np.size(Sa_reg_after_all_down[0,1:-1,:])))), (len(latitude[1:-1]),len(longitude)))
        # top: determine Sa_region of this next timestep 100% stable
        Sa_reg_top[t+1,1:-1,:] = np.reshape(np.maximum(0,np.minimum(np.reshape(W_top[t+1,1:-1,:], np.size(W_top[t+1,1:-1,:])), np.reshape(Sa_reg_after_all_top[0,1:-1,:],
                                                np.size(Sa_reg_after_all_top[0,1:-1,:])))), (len(latitude[1:-1]),len(longitude)))
   
    return Sa_reg_top,Sa_reg_down,north_loss,south_loss,down_to_top,top_to_down,water_lost

#%% Code 
#def get_Sa_reg_backward_TIME(latitude,longitude,count_time,divt,timestep,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,Fa_Vert,E,P,
#                                            W_top,W_down,Sa_reg_top_last,Sa_reg_down_last,Sa_time_top_last,Sa_time_down_last):

# I didn't use this piece of code in this study

#%% create empty array for reg and time
...
def create_empty_array(count_time,divt,latitude,longitude,years):
    ...
    Sa_reg_top = np.zeros((np.int(count_time*divt)+1,len(latitude),len(longitude)))
    Sa_reg_down = np.zeros((np.int(count_time*divt)+1,len(latitude),len(longitude)))
    
    np.savez_compressed(datapathea[0], Sa_reg_top_last=Sa_reg_top[0,:,:], Sa_reg_down_last=Sa_reg_down[0,:,:])    
    np.savez_compressed(datapathea[1], Sa_time_top=Sa_time_top, Sa_time_down=Sa_time_down)    

#    sio.savemat(datapathea[0], {'Sa_reg_top':Sa_reg_top,'Sa_reg_down':Sa_reg_down},do_compression=True)
...
for date in datelist[1:]:
    ...
    print (datapath[0])    
    #Imme: Hier laad je de geregte data van de laatste tijdstap, als de laatste tijdstap er neit was dan is die aangemaakt met create_empty_array en zit ie vol met zeros
    loading_ST = np.load(datapath[0])
    #Sa_reg_top = loading_ST['Sa_reg_top'] # array with zeros #Imme moeten dit zeros zijn of al ingevulde data
    #Sa_reg_down = loading_ST['Sa_reg_down']
    Sa_reg_top_last_1 = loading_ST['Sa_reg_top_last'] #Sa_reg_top[0,:,:]
    Sa_reg_down_last_1 = loading_ST['Sa_reg_down_last'] #Sa_reg_down[0,:,:]
    Sa_reg_top_last =  np.reshape(Sa_reg_top_last_1, (1,len(latitude),len(longitude))) # in deze array staan nan en volgens mij hoort dat niet!!
    Sa_reg_down_last =  np.reshape(Sa_reg_down_last_1, (1,len(latitude),len(longitude))) # in deze array staan nan en volgens mij hoort dat niet!!
    #Sa_reg_top_last =  np.zeros((1, len(latitude), len(longitude)))    
    #Sa_reg_down_last = np.zeros((1, len(latitude), len(longitude))) 
    ...
    Fa_Vert = loading_FS['Fa_Vert']
        ...
        # call the backward reging function
    if timereging == 0: # I use timereging = 0
        Sa_reg_top,Sa_reg_down,north_loss,south_loss,down_to_top,top_to_down,water_lost = get_Sa_reg_forward(latitude,longitude,count_time,divt,Kvf,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
                                       Fa_Vert,E,P,W_top,W_down,Sa_reg_top_last,Sa_reg_down_last)
#    elif timereging == 1:
...
#        
#        Sa_time_top,Sa_time_down,Sa_reg_top,Sa_reg_down,north_loss,south_loss,down_to_top,top_to_down,water_lost = get_Sa_reg_backward_TIME(latitude,longitude,count_time,divt,
#                                        timestep,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,Fa_Vert,E,P,W_top,W_down,Sa_reg_top_last,Sa_reg_down_last,Sa_time_top_last,Sa_time_down_last)
    # save this data 
    #sio.savemat(datapath[3], {'Sa_reg_top':Sa_reg_top,'Sa_reg_down':Sa_reg_down,'north_loss':north_loss, 'south_loss':south_loss,'down_to_top':down_to_top,'top_to_down':top_to_down,'water_lost':water_lost},do_compression=True)
    
    # compute reged evaporation
    Sa_reg = Sa_reg_top + Sa_reg_down
    W = W_top + W_down
    P_reg = P[:,:,:] * (Sa_reg[:-1,:,:] / W[:-1,:,:])
            
    # save per day
    E_per_day = np.sum(E, axis =0)
    P_reg_per_day = np.sum(P_reg, axis =0)
    P_per_day = np.sum(P, axis =0)
    Sa_reg_down_per_day = np.mean(Sa_reg_down[:-1,:,:], axis =0)
    Sa_reg_top_per_day = np.mean(Sa_reg_top[:-1,:,:], axis =0)
    ...
    np.savez_compressed(datapath[3], Sa_reg_top_last = Sa_reg_top[-1,:,:], Sa_reg_down_last = Sa_reg_down[-1,:,:], E_per_day = E_per_day, P_reg_per_day = P_reg_per_day, P_per_day = P_per_day, \
    Sa_reg_top_per_day=Sa_reg_top_per_day, Sa_reg_down_per_day=Sa_reg_down_per_day, W_down_per_day = W_down_per_day, W_top_per_day = W_top_per_day, \
...
#        sio.savemat(datapath[4], {'Sa_time_top':Sa_time_top,'Sa_time_down':Sa_time_down},do_compression=True)
   ...
   # print ('Runtime Sa_reg for day ' + str(a) + ' in month ' + str(monthnumber) +  ' in year ' + str(yearnumber) + ' is',(end - start),' seconds.')
